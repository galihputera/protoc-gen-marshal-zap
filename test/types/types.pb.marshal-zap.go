...
func (x *Types) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	...
	repeated_timestamp_valArrMarshaller := func(enc zapcore.ArrayEncoder) error {
		for _, v := range x.RepeatedTimestampVal {
			if v != nil {
				enc.AppendString(v.AsTime().Format("2006-01-02T15:04:05.999999999Z07:00"))
			} else {
	...
	repeated_duration_valArrMarshaller := func(enc zapcore.ArrayEncoder) error {
		for _, v := range x.RepeatedDurationVal {
			if v != nil {
				enc.AppendString(v.AsDuration().String())
			} else {
				enc.AppendReflected(nil)
			}
		}
	...
	repeated_string_wrapper_valArrMarshaller := func(enc zapcore.ArrayEncoder) error {
		for _, v := range x.RepeatedStringWrapperVal {
			if v != nil {
	...
	enc.AddArray("repeated_string_wrapper_val", zapcore.ArrayMarshalerFunc(repeated_string_wrapper_valArrMarshaller))
	...
	enc.AddObject("map_string_wrapper_val", zapcore.ObjectMarshalerFunc(func(enc zapcore.ObjectEncoder) error {
